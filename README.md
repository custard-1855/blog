...
サイトのソースコード（`src/` 以下のレイアウト・プラグイン・CSS など）は [MIT License](./LICENSE)。
自由にフォーク・改変・再配布してよい。

**記事本文（`src/content/posts/` の Markdown）と About ページの内容は対象外で、全著作権保持。**
コードのライセンスがリポジトリ全体に及ぶわけではないので、記事の転載・改変には別途許諾が必要。
